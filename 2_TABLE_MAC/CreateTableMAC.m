...
%% Create TABLE_NEW_MAC
PER_list        = (0:0.05:0.95);             % Do not modify
PSDULength_list = (100:100:600);             % Do not modify
Alpha_list      = (0:0.1:1);                 % Do not modify
experiment      = 'PSDULength';              % Do not modify
conf.Plotting   = false;                     % Do not modify
conf.SaveTable  = true;                      % Do not modify
conf.TablePath = '../';                      % Table Path
conf.TableName = 'TABLE_NEW_MAC_testing';    % Table Name
CreateMacTable(experiment, Alpha_list, PSDULength_list, PER_list, conf);

%% Closing Clause
runnable = false;

%% function Create TABLE_NEW_MAC
% Do not modify the code in this function. The way the data is store
% impacts the way the Throughput is calculated in EFi_main and may
% lead to undesirable results.
function [] = CreateMacTable(experiment, Alpha_list, PSDULength_list, PER_list, conf)
    varList    = PSDULength_list;
    THTXTIMELIST = zeros(length(Alpha_list),length(PSDULength_list)*length(PER_list));
    for alpha = Alpha_list
        fprintf('\tAlpha = %.2f\n',alpha);
        conf.Alpha = alpha;
        [out] = Throughput_experiment(experiment, varList, PER_list, conf);
        THTXTIMELIST(alpha==Alpha_list,:) = out.THTXTIMELIST;
    end
    THPERLIST = out.THPERLIST;          %#ok % Variable being stored
    THPSDULIST = out.THPSDULIST;        %#ok % Variable being stored
    THTXNUMBLIST = out.THTXNUMBLIST;    %#ok % Variable being stored
    THPDROPLIST = out.THPDROPLIST;      %#ok % Variable being stored
    ALPHAORG = Alpha_list;              %#ok % Variable being stored
    
    tableName = save(strcat(conf.TablePath,conf.TableName));
    save(tableName,'THPERLIST','THPSDULIST',...
                   'THTXNUMBLIST','THPDROPLIST','THTXTIMELIST','ALPHAORG');
end

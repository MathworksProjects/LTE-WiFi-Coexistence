function [avThpt,avPSR,nodes,PSR,PSR_new,...
          posNodes,outFeasExp,K_average,Ktot,...
          totTx] = EFi_function(conf)
...
% Load the CONTENTION Table if not done before
if(~isfield(conf,'TXTIMECONTENT'))
    load('../0_DATA/TABLE_AV_TX_CONTENTION.mat')
else
    TXTIMECONTENT = conf.TXTIMECONTENT;
end
...
if((~isempty(GOCList) && ~isempty(P2PCList)))
    ...
    % Average node count per cluster
%     list = [GOCList -1];
    list = GOCList;
    Ktot = [];
    for relayNode = list
        Ktot = [Ktot length(find(relay==relayNode))];  %#ok<AGROW>
    end
    Ktot = [Ktot 2.*ones(1,length(GOCList))];
    K_average = (sum(Ktot)-length(GOCList))/length(list);
else
    %% Final Node Categorization
    PSR_new = PSR;
...
txTimeGO = zeros(length(GO),1);
txTimeP2P_new = zeros(length(P2P),1);
txTimeCSZ = zeros(length(Clients_SZ),1);
txTimeCNSZ = zeros(length(Clients_NSZ),1);
txTimeGeneral = zeros(Nnodes,1);
txTimeGeneralLTEU = zeros(Nnodes,1);
for go = GO
    thptGO(go==GO) = calculateThroughputClient(conf, ...
        PSR(go), THPERLIST, THPSDULIST, THTXTIMELIST, rate);
    txTimeGeneral(go) = TXTIMECONTENT(Nnodes,2)/PSR(go);
    txTimeGeneralLTEU(go) = TXTIMECONTENT(Nnodes,1)/PSR(go);
    nDevContending = length(find(relay_final==go));
    txTimeGO(go==GO) = TXTIMECONTENT(nDevContending,2)/PSR(go);
end
for p2p = P2P    
    ...
    txTimeGeneral(p2p) = TXTIMECONTENT(Nnodes,2)/PSR(p2p);
    txTimeGeneralLTEU(p2p) = TXTIMECONTENT(Nnodes,1)/PSR(p2p);
    nDevContending = length(find(relay_final==relay(p2p)));
    txTimeP2P_new(p2p==P2P) = TXTIMECONTENT(nDevContending,2)/PSR_new(p2p);
end
for csz = Clients_SZ
    thptCSZ(csz==Clients_SZ) = calculateThroughputClient(conf, ...
        PSR(csz), THPERLIST, THPSDULIST, THTXTIMELIST, rate);
    txTimeGeneral(csz) = TXTIMECONTENT(Nnodes,2)/PSR(csz);
    txTimeGeneralLTEU(csz) = TXTIMECONTENT(Nnodes,1)/PSR(csz);
    nDevContending = length(setdiff(relay_final,GO));
    txTimeCSZ(csz==Clients_SZ) = TXTIMECONTENT(nDevContending,2)/PSR(csz);
end
for cnsz = Clients_NSZ
    ...
    txTimeGeneral(cnsz) = TXTIMECONTENT(Nnodes,2)/PSR(cnsz);
    txTimeGeneralLTEU(cnsz) = TXTIMECONTENT(Nnodes,1)/PSR(cnsz);
    nDevContending = length(setdiff(relay_final,GO));
    txTimeCNSZ(cnsz==Clients_NSZ) = TXTIMECONTENT(nDevContending,2)/PSR(cnsz);
end

% Compute the number of ABS needed per group (nodes that contend)
nReqABS = zeros(length(GO)+1,1);
for rl = GO
    timeVector = [];
    timeVector = [timeVector txTimeGO(GO==rl)];  %#ok
    for p2p = P2P
        if (relay_final(p2p)==rl) && (txTimeP2P_new(P2P==p2p)~=inf)
            timeVector = [timeVector txTimeP2P_new(P2P==p2p)];  %#ok
        end
    end
    timeVector(timeVector==inf) = [];  % The system avoids infinites
    nReqABS(GO==rl) = ceil(max(timeVector));
end
% Compute the number of ABS needed in Group Cc (CSZ and CNSZ)
txTimeCSZ(txTimeCSZ==inf) = [];
txTimeCNSZ(txTimeCNSZ==inf) = [];
if ~isempty(txTimeCSZ) || ~isempty(txTimeCNSZ)  % Check if no Cc group
    nReqABS(end) = ceil(mean([txTimeCSZ;txTimeCNSZ]));
    nReqABSTot = sum(nReqABS);
else
    nReqABSTot = sum(nReqABS(1:end-1));
end

% Compare Required time to transmit between E-Fi and LTE-U,
% assuming Ton and Toff to be 80ms. This means 80ms are allocated to WiFi 
% with no LTE interference present and 80ms are allocated to LTE. As for
% E-Fi, the number of available ABS define the total required time.
TLTEUon = 80;  % Period (ms) WiFi is active and can contend for the channel
txTimeGeneralLTEU(txTimeGeneralLTEU==inf) = [];
nLTEUBlocks = ceil(max(txTimeGeneralLTEU)/TLTEUon);
if nLTEUBlocks==1
    totTx.LTEU = max(txTimeGeneralLTEU);
else
    totTx.LTEU = nLTEUBlocks*2*TLTEUon;
end
NABSPerFrame = (1:1:10);  % Compute for all possible values
totTx.EFi = nReqABSTot .* (10 ./ NABSPerFrame);


% Compute average Throughput per Node Category
